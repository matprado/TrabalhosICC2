...
int compara_palavras(char *p1, char *p2){
	...
	for(int i=0, j=0; i<s1 && j<s2; i++, j++){
		if(maiuscula(p1[i]) != maiuscula(p2[j])){
			return (int)maiuscula(p1[i]) - (int)maiuscula(p2[j]);
		}
	}
...
void merge(char **vet, int ini, int meio, int fim){
    ...
    for(i=ini, j=meio+1, k=0; i<=meio && j<=fim; k++){	//Ordena os dois 'blocos'
    	if(compara_palavras(vet[i], vet[j]) < 0)
    		strcpy(aux[k], vet[i++]);
    	else
    		strcpy(aux[k], vet[j++]);
    }
...
char *ler_palavra(FILE *arq, char PARA){
	...
	ascii = (int)c;
	while((ascii < 65 || (ascii > 90 && ascii < 97) || ascii > 122) && c != EOF && c != PARA){
		c = fgetc(arq);
		ascii = (int)c;	
	}
...
dicionario *ler_tweets(FILE *arq, int *n){
	...
	//size_t size;
	while((c = fgetc(arq)) != EOF){
		while(c == ' '){	/*Ignorar espaços*/
			c = fgetc(arq);
		}
		if(c == '"' && (c = fgetc(arq)) == 't' && (c = fgetc(arq)) == 'e' && (c = fgetc(arq)) == 'x' && (c = fgetc(arq)) == 't'){
			twitter = (dicionario *) realloc(twitter, ++aux * sizeof(dicionario));
			twitter[aux-1].palavras = (char **) malloc(MAX_TWEET * sizeof(char*));
			twitter[aux-1].quantidade = 0;
			c = fgetc(arq); // '"'
			c = fgetc(arq); // ':'
			c = fgetc(arq); // ' '
			c = fgetc(arq); // '"'
			while((c = fgetc(arq)) != '\"'){
				fseek(arq, -1L, SEEK_CUR);
				twitter[aux-1].palavras[twitter[aux-1].quantidade++] = ler_palavra(arq, '\"');/*lê uma palavra*/
				fseek(arq, -1L, SEEK_CUR);
			}
		}else{
			while(c != '\n' && c != EOF){
	...
	*n = aux; // modifica a variavel que veio da main
	return twitter;
}
